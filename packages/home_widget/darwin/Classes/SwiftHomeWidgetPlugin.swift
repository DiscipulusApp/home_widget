...
public class SwiftHomeWidgetPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    ...
    // https://github.com/flutter/flutter/issues/118103
      ...
      let messenger = registrar.messenger
        ...
        channel = FlutterMethodChannel(name: "home_widget", binaryMessenger: messenger)
        eventChannel = FlutterEventChannel(name: "home_widget/updates", binaryMessenger: messenger)
    ...
    if call.method == "setAppGroupId" {
        ...
        let name = (myArgs["ios"] ?? myArgs["name"]) as? String
      {
        if #available(iOS 14.0, macOS 11.0, *) {
          ...
          result(
            FlutterError(
              code: "-4", message: "Widgets are only available on iOS 14.0 and above or macOS 11.0 and above", details: nil)
          )
        }
      } else {
        result(
      ...
      if #available(iOS 17.0, macOS 14.0, *) {
        ...
        result(
          FlutterError(
            code: "-5",
            message: "Interactivity is only available on iOS 17.0 or macOS 14.0",
            details: nil))
      }
      ...
      if #available(iOS 14.0, macOS 11.0, *) {
        ...
        result(
          FlutterError(
            code: "-4", message: "Widgets are only available on iOS 14.0 and above or macOS 11.0 and above",
            details: nil)
        )
      }
    } else {
  ...
  private func isWidgetUrl(url: URL) -> Bool {
    ...
    return components?.queryItems?.contains(where: { $0.name == "homeWidget" }) ?? false
}
}
